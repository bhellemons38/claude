...
export function Pricing() {
  return (
    <section id="prijzen" className="relative">
      <div className="pointer-events-none absolute left-1/2 top-1/3 -z-0 h-72 w-[680px] -translate-x-1/2 blur-3xl lp-glow-amber opacity-50" aria-hidden />
      <div className="relative max-w-6xl mx-auto px-4 sm:px-7 py-20 sm:py-28">
        <Reveal className="text-center max-w-2xl mx-auto">
          <span className="lp-pill text-rood">Prijzen</span>
          ...
          <p className="mt-4 text-inkt-zacht">Alle prijzen zijn exclusief 9% BTW. Minimale bestelwaarde € 75 excl. BTW.</p>
        </Reveal>
          ...
          {TIERS.map(tier => (
            <motion.div
              ...
              className={`relative flex flex-col rounded-3xl p-7 ${
                tier.highlight ? 'lp-ring bg-wit md:-mt-4 md:mb-4 shadow-[0_24px_50px_-24px_rgba(28,20,8,0.4)]' : 'lp-glass lp-glass-hover'
              }`}
            >
              {tier.highlight && (
                <span className="lp-pill absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-rood/30 bg-rood/12 px-3 py-1 text-rood">
                  Meest gekozen
                </span>
              )}
              <h3 className="font-geist text-lg font-semibold text-inkt">{tier.name}</h3>
              <div className="mt-3 flex items-baseline gap-2">
                <span className="font-geist text-4xl font-semibold tracking-tight text-inkt">{tier.price}</span>
                <span className="lp-pill text-inkt-zacht">{tier.unit}</span>
              </div>
              <p className="mt-3 text-sm text-inkt-zacht leading-relaxed">{tier.description}</p>

              <ul className="mt-6 flex-1 space-y-3">
                {tier.features.map(feature => (
                  <li key={feature} className="flex items-start gap-2.5 text-sm text-inkt/80">
                    <svg viewBox="0 0 20 20" className="mt-0.5 h-4 w-4 flex-none text-rood" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden>
              ...
              <Link
                ...
                className={`mt-7 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-semibold transition-transform hover:scale-[1.02] active:scale-95 ${
                  tier.highlight ? 'bg-rood text-wit' : 'border border-inkt/15 bg-wit/60 text-inkt hover:bg-wit'
                }`}
              >
                {tier.cta}
              </Link>
